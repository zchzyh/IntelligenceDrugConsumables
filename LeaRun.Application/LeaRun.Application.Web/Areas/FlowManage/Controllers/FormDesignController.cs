        ...
        public ActionResult WebUploadFile()
        {
            ...
            try
            {
                for (int i = 0; i < Request.Files.Count; i++)
                {
                    ...
                    string virtualPath = string.Format("~/Uploads/DocumentFile/{0}/{1}/{2}{3}", userId, uploadDate, fileGuid, FileEextension);
                    string fullFileName = this.Server.MapPath(virtualPath);
                    string path = Path.GetDirectoryName(fullFileName);
                    // 文件上传后的保存路径
            ...
            {
                return Json(new { Success = "NO", Message = ex.Message }, JsonRequestBehavior.AllowGet);
            }
            return Json(new { Success = "NO", Message = "请选择要上传的文件！" }, JsonRequestBehavior.AllowGet);
        }
