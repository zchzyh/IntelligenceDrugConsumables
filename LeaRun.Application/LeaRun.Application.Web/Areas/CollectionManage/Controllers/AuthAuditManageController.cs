...
namespace LeaRun.Application.Web.Areas.CollectionManage.Controllers
{
    ...
    /// 采集权限分配
    /// </summary>
    public class AuthAuditManageController : MvcControllerBase
    {
        private readonly BpcSp004BLL _bpcSp004Bll = new BpcSp004BLL();
        ...
        public ActionResult GetPageList(Pagination pagination, string queryJson)
        {
            ...
            var list = _bpcSp004Bll.GetPageList(pagination, queryJson);
            var jsonData = new
            {
                rows = list,
                pagination.total,
        ...
        public ActionResult SaveData(string userId, string year, string tableIds)
        {
            ...
            var entities = new List<BpcSp004Entity>();
            ...
            foreach (var id in tbids)
            {
                ...
                BpcSp004Entity entity = new BpcSp004Entity {ND = year, CJBBM = id};
                entities.Add(entity);
            }

            _bpcSp004Bll.SaveData(year, userId, entities);
            return Success("操作成功");
        }
    }
}